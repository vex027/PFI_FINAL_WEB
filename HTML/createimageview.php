<?php 
    ...
    if($_SESSION["userID"] != $album->get_authorID()){
        header("Location: error.php?ErrorMSG=Pas la permission de créer d'image sur cette album !");
        die();
    } 
?>

<div class="container align-middle border mb-sm-5">
    <h2> Ajouter un image à l'album : <?php echo $album->get_title()?>  </h2>
    <form class method = "post" action = "./DOMAINLOGIC/createimage.dom.php?id=<?php echo $album->get_id()?>" enctype="multipart/form-data">
        ...
        <button class='btn btn-success' type="submit">Ajouter un image</button>;
    </form>
</div>